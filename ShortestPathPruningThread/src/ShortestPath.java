...
public class ShortestPath {
	...
	public void NodeSplit (){
		...
		// begin to split the node into 2 child node
		/*
		System.out.println();
		System.out.println("Node being split name : "+ temp.getM_name());						
		System.out.println("Node being split cost : "+ temp.getM_value());
		System.out.print ("Node being split remaining edges: ");
		for (int i = 0; i < temp.getM_edges().size(); i++){
			System.out.print(temp.getM_edges().get(i)+", ");
		}
		System.out.println();
	...
	public synchronized void SmalletcostCheck(Node temp){
	...
	public synchronized boolean Pruning (Node temp){
		if(getSmallestCost() != 0.0 && temp.getM_value() >= getSmallestCost()){
			//System.out.println("path is pruned and it cost is: "+temp.getM_value());
			//System.out.println();
			return true;
		}
		return false;
